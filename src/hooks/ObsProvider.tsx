...
export function obsToInternal(
  ...
  let name = stripped.replace(/[·•]\s*(16:9|16:10)\s*$/, (_, a) => {
...
export function ObsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const refreshAudioInputs = async () => {
      try {
        ...
        const audio = (inputs as { inputName: string; inputKind: string }[]).filter((i) =>
          AUDIO_KINDS.has(i.inputKind),
        );
    ...
    const tryConnect = async () => {
      ...
      try {
        ...
        const [{ currentProgramSceneName }, { scenes }, streamStatus, recordStatus] =
          await Promise.all([
    ...
    obs.on("CurrentProgramSceneChanged", ({ sceneName }) => setObsScene(sceneName));
    ...
    obs.on("StreamStateChanged", ({ outputActive }) => setStreaming(outputActive));
    obs.on("RecordStateChanged", ({ outputActive }) => setRecording(outputActive));
    ...
    obs.on("InputVolumeChanged", ({ inputName, inputVolumeDb }) =>
      setAudioInputs((prev) =>
        prev.map((i) => (i.name === inputName ? { ...i, volumeDb: inputVolumeDb } : i)),
      ),
    );
    obs.on("InputMuteStateChanged", ({ inputName, inputMuted }) =>
      setAudioInputs((prev) =>
        prev.map((i) => (i.name === inputName ? { ...i, muted: inputMuted } : i)),
      ),
    );
    ...
    obs.on("InputVolumeMeters", ({ inputs }) => {
      for (const i of inputs as { inputName: string; inputLevelsMul: number[][] }[]) {
    ...
    flushTimer.current = setInterval(() => {
      ...
      setAudioInputs((prev) =>
        prev.map((i) => ({ ...i, level: snapshot.get(i.name) ?? i.level * 0.7 })),
      );
    }, 50);
  ...
  const setScene = async (obsName: string) =>
    void (await safeCall(() => wsRef.current!.call("SetCurrentProgramScene", { sceneName: obsName })));
  ...
  const toggleMute = async (inputName: string) =>
    void (await safeCall(() => wsRef.current!.call("ToggleInputMute", { inputName })));

  const triggerHotkey = async (name: string) =>
    void (await safeCall(() => wsRef.current!.call("TriggerHotkeyByName", { hotkeyName: name })));
